...
//***************************************************************
/*
 *  $Id: ColorChooser.java,v 1.17 2007/01/15 11:38:28 rudahl Exp $
 *  $Log: ColorChooser.java,v $
 *  Revision 1.17  2007/01/15 11:38:28  rudahl
 *  added paragraph to help
 *
 *  Revision 1.16  2005/09/16 11:50:46  goldin
...
public class ColorChooser extends JniDialog
    ...
    protected static String[] helpKeys = { "%h6509.2",
       ...
       */
    public ColorChooser(I18NTextSource textSource)
        { 
        super(textSource, "COLTITLE??");
	...
	if (!bOk)
	    {
	    /*
	    viewProg.logViewport("Error finding or reading color file " +
			this.baseClfFile);
	    */
   ...
   public void dismissChooser()
	...
	while (clsNames[maxClass].length() == 0)
            maxClass -= 1;
	...
	viewProg.logViewport("Mouse moved - source is " + source);
	if (source == resultCanvas)
	    {
            int index = resultCanvas.getMouseIndex(e);
	    viewProg.logViewport(
		"In resultcanvas, index is "+String.valueOf(index));
            if (index >= 0)
    ...
    public void mouseExited(MouseEvent e)
        {
	viewProg.logViewport("Mouse exited");
	currentItemLabel.setText(colorIndexString + " ---");
        }
	...
	int iEnd = -1;
		...
		viewProg.logViewport("Setting start to " +
				     String.valueOf(iStart));
    ...
    protected void setNewColor(int start, int end)
        {
	viewProg.logViewport("setNewColor entered: start=" + 
	...
	pIT.showIt();
	viewProg.logViewport("Just showed the info text box");
	...
	viewProg.logViewport(
	   "In ColorChooser.actionPerformed, source is " + source);
	    ...
	    setVisible(false);
            if (bOk)
	       {
	       viewProg.logViewport("ColorChooser Save button pressed");
    	       viewProg.SendViewportEvent(ae.getActionCommand());
	       }
	    else
	       {
	       viewProg.logViewport("ColorChooser Save colors error");
    	       viewProg.SendViewportEvent(cancelButton.getActionCommand());
	       }
	    }
        else if (source.equals(helpButton))
	    {
	    viewProg.logViewport("ColorChooser Help button clicked");
	    showHelp(helpKeys,false);
	    }
        else if (source.equals(cancelButton))
	    {
	    setVisible(false);
	    viewProg.logViewport("ColorChooser Cancel button pressed");
    	    viewProg.SendViewportEvent(ae.getActionCommand());
	    }
